...
use fedimint_ln::config::SimpleClientConfig;
...
use fedimint_ln::SimpleGen;
use fedimint_ln::{
    ContractOutput, Simple, SimpleError, SimpleInput, SimpleOutput,
    SimpleOutputOutcome,
};
...
async fn test_account() {
    ...
    let mut fed = FakeFed::<Simple>::new(
        4,
        |cfg, _db| async move { Ok(Simple::new(cfg.to_typed()?)) },
        &ConfigGenParams::new(),
        &SimpleGen,
    ...
    let account_output = SimpleOutput::Contract(ContractOutput {
    ...
    match fed.output_outcome(account_out_point).await.unwrap() {
        SimpleOutputOutcome::Contract { outcome, .. } => {
            assert_eq!(outcome, ContractOutcome::Account(AccountContractOutcome {}));
        }
        _ => panic!(),
    };

    let account_input = SimpleInput {
...
async fn test_outgoing() {
    ...
    let invoice: Simple_invoice::Invoice =
...
j5r6drg6k6zcqj0fcwg"
    ...
    let outgoing_output = SimpleOutput::Contract(ContractOutput {
        amount: Amount::from_sats(42),
        contract: contract.clone(),
    });
    ...
    let account_input_no_witness = SimpleInput {
        contract_id: contract.contract_id(),
        amount: Amount::from_sats(42),
        witness: None,
    };
    ...
    assert_eq!(
        format!("{err}"),
        format!("{}", SimpleError::MissingPreimage)
    );

    // Ok
    let account_input_witness = SimpleInput {
        contract_id: contract.contract_id(),
        amount: Amount::from_sats(42),
...
async fn test_incoming() {
    ...
    let offer = IncomingContractOffer {
        ...
        encrypted_preimage: EncryptedPreimage::new(
            preimage.clone(),
            &fed.client_cfg_typed::<SimpleClientConfig>()
                .unwrap()
                .threshold_pub_key,
        ),
        expiry_time: None,
    };
    let offer_output = SimpleOutput::Offer(offer.clone());
    ...
    let incoming_output = SimpleOutput::Contract(ContractOutput {
        amount: Amount::from_sats(42),
        contract: contract.clone(),
    });
    ...
    let incoming_input = SimpleInput {
        contract_id: contract.contract_id(),
        amount: Amount::from_sats(42),
        witness: None,
    };
    ...
    assert_eq!(
        format!("{error}"),
        format!("{}", SimpleError::ContractNotReady)
    );
