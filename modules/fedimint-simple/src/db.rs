...
use crate::{ContractAccount, SimpleGateway, SimpleOutputOutcome};
...
impl DatabaseKeyPrefixConst for ContractUpdateKey {
    ...
    type Key = Self;
    type Value = SimpleOutputOutcome;
}
...
#[derive(Debug, Encodable, Decodable, Serialize)]
pub struct SimpleGatewayKey(pub PublicKey);

impl DatabaseKeyPrefixConst for SimpleGatewayKey {
    const DB_PREFIX: u8 = DbKeyPrefix::SimpleGateway as u8;
    type Key = Self;
    type Value = SimpleGateway;
}

#[derive(Debug, Encodable, Decodable)]
pub struct SimpleGatewayKeyPrefix;

impl DatabaseKeyPrefixConst for SimpleGatewayKeyPrefix {
    const DB_PREFIX: u8 = DbKeyPrefix::SimpleGateway as u8;
    type Key = SimpleGatewayKey;
    type Value = SimpleGateway;
}
