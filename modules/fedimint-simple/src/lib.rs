...
//! the Simple network through a Simple gateway. See [`Simple`] for a high level
//! overview.
//!
...
use db::{SimpleGatewayKey, SimpleGatewayKeyPrefix};
...
use crate::common::SimpleDecoder;
use crate::config::{
    SimpleClientConfig, SimpleConfig, SimpleConfigConsensus, SimpleConfigPrivate,
};
use crate::contracts::{
...
/// The Simple module implements an account system. It does not have the privacy guarantees of
...
///     Simple node to make payments for the funder
...
///
/// These three primitives allow to integrate the federation with the wider Simple network
/// through a centralized but untrusted (except for availability) Simple gateway server.
///
...
impl std::fmt::Display for SimpleInput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Simple Contract {} with amount {}",
            self.contract_id, self.amount
        )
    }
}

/// Represents an output of the Simple module.
///
...
impl std::fmt::Display for SimpleOutput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SimpleOutput::Contract(ContractOutput { amount, contract }) => match contract {
            ...
            SimpleOutput::Offer(offer) => {
                write!(f, "LN offer for {} with hash {}", offer.amount, offer.hash)
            }
            SimpleOutput::CancelOutgoing { contract, .. } => {
...
impl std::fmt::Display for SimpleOutputOutcome {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SimpleOutputOutcome::Contract { id, .. } => {
                write!(f, "LN Contract {}", id)
            }
            SimpleOutputOutcome::Offer { id } => {
                write!(f, "LN Offer {}", id)
            }
        }
    }
...
pub struct SimpleConsensusItem {
    pub contract_id: ContractId,
    pub share: PreimageDecryptionShare,
}

impl std::fmt::Display for SimpleConsensusItem {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
#[derive(Debug, Clone, PartialEq, Eq, Hash, Encodable, Decodable, Serialize, Deserialize)]
pub struct SimpleVerificationCache;

#[derive(Debug)]
pub struct SimpleGen;

#[async_trait]
impl ModuleGen for SimpleGen {
    const KIND: ModuleKind = KIND;
    type Decoder = SimpleDecoder;

    fn decoder(&self) -> SimpleDecoder {
        SimpleDecoder
    }
    ...
    async fn init(
        ...
        Ok(Simple::new(cfg.to_typed()?).into())
    }
        ...
        let server_cfg = peers
            ...
            .map(|&peer| {
                ...
                (
                    peer,
                    SimpleConfig {
                        consensus: SimpleConfigConsensus {
                            threshold_pub_keys: pks.clone(),
                            fee_consensus: FeeConsensus::default(),
                        },
                        private: SimpleConfigPrivate {
    ...
    fn to_config_response(
        ...
        let config = serde_json::from_value::<SimpleConfigConsensus>(config)?;
    ...
    fn hash_client_module(
        ...
        serde_json::from_value::<SimpleClientConfig>(config)?.consensus_hash()
    }
}

#[async_trait]
impl ServerModule for Simple {
    const KIND: ModuleKind = KIND;
    ...
    type VerificationCache = SimpleVerificationCache;
    ...
    async fn consensus_proposal(
        ...
        dbtx.find_by_prefix(&ProposeDecryptionShareKeyPrefix)
            ...
            .map(|res| {
                ...
                SimpleConsensusItem { contract_id, share }
            })
            .collect()
    }
    ...
    async fn validate_input<'a, 'b>(
        ...
        let account: ContractAccount = self
            ...
            .await
            .ok_or(SimpleError::UnknownContract(input.contract_id))
            .into_module_error_other()?;

        if account.amount < input.amount {
            return Err(SimpleError::InsufficientFunds(
                account.amount,
                input.amount,
            ))
            .into_module_error_other();
        ...
        let pub_key = match account.contract {
                ...
                if outgoing.timelock > block_height(interconnect).await && !outgoing.cancelled {
                    ...
                    let preimage_hash = bitcoin_hashes::sha256::Hash::hash(
                        &input
                            .witness
                            .as_ref()
                            .ok_or(SimpleError::MissingPreimage)
                            .into_module_error_other()?
                            .0,
                    );
                    ...
                    if preimage_hash != outgoing.hash {
                        return Err(SimpleError::InvalidPreimage).into_module_error_other();
                    }
            ...
            FundedContract::Incoming(incoming) => match incoming.contract.decrypted_preimage {
                ...
                DecryptedPreimage::Pending => {
                    return Err(SimpleError::ContractNotReady).into_module_error_other();
                }
    ...
    async fn validate_output(
        ...
        match output {
            SimpleOutput::Contract(contract) => {
                ...
                if let Contract::Incoming(incoming) = &contract.contract {
                    let offer = dbtx
                        ...
                        .await
                        .expect("DB error")
                        .ok_or(SimpleError::NoOffer(incoming.hash))
                        .into_module_error_other()?;

                    if contract.amount < offer.amount {
                        // If the account is not sufficiently funded fail the output
                        return Err(SimpleError::InsufficientIncomingFunding(
                            offer.amount,
                            contract.amount,
                        ))
                        .into_module_error_other();
                ...
                if contract.amount == Amount::ZERO {
                    Err(SimpleError::ZeroOutput).into_module_error_other()
                } else {
            ...
            SimpleOutput::Offer(offer) => {
                if !offer.encrypted_preimage.0.verify() {
                    Err(SimpleError::InvalidEncryptedPreimage).into_module_error_other()
                } else {
                    Ok(TransactionItemAmount::ZERO)
                }
            }
            SimpleOutput::CancelOutgoing {
                contract,
                gateway_signature,
            } => {
                let contract_account = dbtx
                    ...
                    .await
                    .expect("DB error")
                    .ok_or(SimpleError::UnknownContract(*contract))
                    .into_module_error_other()?;

                let outgoing_contract = match &contract_account.contract {
                    FundedContract::Outgoing(contract) => contract,
                    _ => {
                        return Err(SimpleError::NotOutgoingContract).into_module_error_other();
                    }
                };
                ...
                secp256k1::global::SECP256K1
                    ...
                    .map_err(|_| SimpleError::InvalidCancellationSignature)
                    .into_module_error_other()?;
    ...
    async fn apply_output<'a, 'b>(
        ...
        match output {
            ...
            SimpleOutput::Offer(offer) => {
                dbtx.insert_new_entry(
                    &ContractUpdateKey(out_point),
                    &SimpleOutputOutcome::Offer { id: offer.id() },
                )
                .await
                .expect("DB Error");
    ...
    fn api_endpoints(&self) -> Vec<ApiEndpoint<Self>> {
        vec![
            api_endpoint! {
                "/account",
                async |module: &Simple, dbtx, contract_id: ContractId| -> ContractAccount {
                    module
                        .get_contract_account(dbtx, contract_id)
                        .await
            ...
            api_endpoint! {
                "/offers",
                async |module: &Simple, dbtx, _params: ()| -> Vec<IncomingContractOffer> {
                    Ok(module.get_offers(dbtx).await)
                }
            },
            api_endpoint! {
                "/offer",
                async |module: &Simple, dbtx, payment_hash: bitcoin_hashes::sha256::Hash| -> IncomingContractOffer {
                    let offer = module
            ...
            api_endpoint! {
                "/list_gateways",
                async |module: &Simple, dbtx, _v: ()| -> Vec<SimpleGateway> {
                    Ok(module.list_gateways(dbtx).await)
                }
            },
            api_endpoint! {
                "/register_gateway",
                async |module: &Simple, dbtx, gateway: SimpleGateway| -> () {
...
impl Simple {
    pub fn new(cfg: SimpleConfig) -> Self {
        Simple { cfg }
    }
    ...
    pub async fn list_gateways(&self, dbtx: &mut DatabaseTransaction<'_>) -> Vec<SimpleGateway> {
        dbtx.find_by_prefix(&SimpleGatewayKeyPrefix)
            .await
            .map(|res| res.expect("DB error").1)
            .collect()
    }
    ...
    pub async fn register_gateway(
        ...
        dbtx.insert_entry(&SimpleGatewayKey(gateway.node_pub_key), &gateway)
            .await
            .expect("DB error");
    }
}
